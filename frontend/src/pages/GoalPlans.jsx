        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={cn("text-xs px-2 py-0.5 rounded-full", statusPill.className)}>
                {statusPill.label}
              </span>
              <span className={cn(
                "text-xs px-2 py-0.5 rounded-full",
                goal.goalType === "shortTerm" ? "bg-purple-500/10 text-purple-600" : "bg-indigo-500/10 text-indigo-600"
              )}>
                {goal.goalType === "shortTerm" ? "Short" : "Long"}
              </span>
            </div>
            {goal.description ? (
              <CardDescription className="mt-1 line-clamp-2 wrap-break-word">{goal.description}</CardDescription>
            ) : null}
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg border bg-muted/20 p-3">
            ...
            <p className="text-sm font-semibold">₹{Math.max(0, toNumber(goal.targetAmount)).toLocaleString()}</p>
          </div>
          <div className="rounded-lg border bg-muted/20 p-3">
            <p className="text-xs text-muted-foreground">Saved</p>
            <p className="text-sm font-semibold">₹{Math.max(0, toNumber(goal.currentAmount)).toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="rounded-lg border bg-muted/20 p-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              {plan.deadline ? (
                <p className="text-sm font-semibold">By {format(plan.deadline, "MMM d, yyyy")}</p>
              ) : (
                <p className="text-sm font-semibold">Add a deadline for accuracy</p>
              )}
            </div>
          ...
          {plan.perWeek ? (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-[11px] text-muted-foreground">Weekly savings</p>
                <p className={cn(
              ...
              <div>
                ...
                <p className="text-sm font-semibold">₹{plan.remaining.toLocaleString()}</p>
              </div>
            </div>
          ) : (
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant="outline" size="sm" onClick={() => onUpdateAmount(goal)}>
            Update saved
          </Button>
          <Button
