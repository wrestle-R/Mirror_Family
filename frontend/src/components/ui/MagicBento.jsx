...
const MagicBento = ({
  ...
  return (
    ...
    >
      ...
      <div 
        ref={gridRef}
        className="grid gap-2 p-3 max-w-6xl mx-auto pt-12 pb-2
          grid-cols-1 
          sm:grid-cols-2 
          lg:grid-cols-4"
      >
        {cardData.map((card, index) => {
          const cardClasses = `
            ...
            min-h-[200px] w-full p-5 rounded-3xl
            border
            transition-all duration-300 ease-in-out
            hover:-translate-y-1 hover:shadow-xl
            overflow-hidden
            ${index === 2 ? 'lg:col-span-2 lg:row-span-2' : ''}
            ${index === 3 ? 'lg:col-start-1 lg:col-span-2 lg:row-start-2 lg:row-span-2' : ''}
            ${index === 5 ? 'lg:col-start-4 lg:row-start-3' : ''}
            ${enableBorderGlow ? 'hover:shadow-purple-500/20 dark:hover:shadow-purple-500/30' : ''}
          `;

          // use the design tokens from index.css for non-purple colors
          const cardStyle = {
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col relative z-10">
                ...
                <p className={`text-sm leading-tight opacity-90 ${textAutoHide ? 'line-clamp-2' : ''}`}>
                  {card.description}
                </p>
              </div>
